...
[deploy]
startCommand = "gunicorn -w 2 -b 0.0.0.0:$PORT 'craft_unified:create_app_with_db()'"
...
Runs initial sync if database is empty, then starts the server.
"""
import os
import subprocess
import sys

DB_PATH = os.environ.get('DB_PATH', 'craft_unified.db')
...
def run_sync():
    ...
    print("=" * 50)
    print("Running initial data sync from Eventbrite...")
    print("This may take 5-15 minutes.")
    print("=" * 50)
    
    result = subprocess.run([
        sys.executable, 'craft_unified.py', 'sync', API_KEY, '--years', '2'
    ])
    
    if result.returncode != 0:
        print("Warning: Initial sync had issues. Check logs.")

def start_server():
    """Start gunicorn server."""
    print(f"\n🚀 Starting Craft Dominant API on port {PORT}\n")
...
if __name__ == '__main__':
    if not API_KEY:
        print("ERROR: EVENTBRITE_API_KEY not set")
        print("Run: railway variables set EVENTBRITE_API_KEY=your_key")
        sys.exit(1)

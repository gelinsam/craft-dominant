...
class ProviderDraftTests(unittest.TestCase):
    def setUp(self):
        ...
        for target,value in [('provider_drafts.event_audience_id','1234567890'),('provider_drafts.build_crm_audience',{'records':[{'email':'a@example.com'},{'email':'nonconsented@example.com'}]}),('sales_evidence.verified_edition_at',NOW)]:
    ...
    def test_incomplete_sales_never_creates(self):
        with patch('sales_evidence.verified_edition_at',return_value=None):
            with self.assertRaisesRegex(ValueError,'sales refresh'):self.prepare()
